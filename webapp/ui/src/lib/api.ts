...
export async function listCodes(typology: string, parameterId: string) {
  const raw = await json<unknown[]>(
    `/api/typologies/${encodeURIComponent(typology)}/parameters/${encodeURIComponent(parameterId)}/codes`,
  );
...
export async function listLanguageColumns(typology: string) {
  const raw = await json<unknown[]>(
    `/api/typologies/${encodeURIComponent(typology)}/languages/columns`,
  );
...
export async function listColumnValues(typology: string, column: string, limit = 500) {
  return await json<string[]>(
    `/api/typologies/${encodeURIComponent(typology)}/languages/values?column=${encodeURIComponent(column)}&limit=${limit}`,
  );
}
...
export async function runQuery(req: QueryRequest): Promise<QueryResult> {
  ...
  const raw = await json<unknown>("/api/query", {
    method: "POST",
...
export async function compareEstimators(body: {
  ...
  const raw = await json<unknown>("/api/compare-estimators", {
    method: "POST",
    body: JSON.stringify({
...
export async function crossValidate(body: {
  ...
  const raw = await json<unknown>("/api/cross-validate", {
    method: "POST",
    body: JSON.stringify({
      typology: body.typology,
      target: body.target,
...
export async function rankAssociations(body: {
  ...
  const raw = await json<unknown>("/api/rank-associations", {
    method: "POST",
    body: JSON.stringify({
      typology: body.typology,
      target: body.target,
